                ...
                <table class="table table-bordered datatable" id="">
                	<thead>
                		<tr>
                            ...
                            <th><div><?php echo get_phrase('cycle');?></div></th>
                    ...
                    <tbody>
                    	<?php $count = 1;foreach($classes as $row):
                            $cycle_id=$row['cycle'];
                            //$cycle_id   =$this->db->get_where('class_cycle',array('class_id'=>$row['class_id'],'year'=>$running_year))->row()->cycle;
                            $cycle_name = $this->db->get_where('school_fees',array('id'=>$cycle_id))->row()->name;   
                        ?>
                        <tr>
                            <td><?php echo $count++;?></td>
							<td><?php echo $row['name'];?></td>
							<td><?php echo $row['name_numeric'];?></td>
                            <td><?php echo $cycle_name; ?></td>
							<td>
                                <?php
                	...
                	<?php echo form_open(site_url('admin/classes/create') , array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>
                        <div class="padded">
                            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <select name="cycle" class="form-control select2" style="width:100%;" data-validate="required" data-message-required="<?php echo get_phrase('value_required');?>">
                                        <option value=""><?php echo get_phrase('select_cycle');?></option>
                                        <?php  foreach($cycles as $key){?>
