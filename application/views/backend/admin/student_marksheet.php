...
foreach ($student_info as $row1):
    foreach ($exams as $row2):
            ...
            foreach ($marks as $row4) {
                if ($row4['subject_id'] == $row3['subject_id']) {
                    if (!empty($row4['test2']) AND!empty($row4['mark_obtained'])) {
                        echo $total = ( $row4['test2'] * 0.7 + $row4['mark_obtained'] * 0.3);
                    }
                }
            }
            ?>
                                            ...
                                            <td style="text-align: center;">
                                                <?php
                                                echo $grade = $this->db->get_where('grade',
                                                        array('mark_from <=' => $total, 'mark_upto >=' => $total, 'lang' => 'fr'))->row()->grade_point;
                                                ?>
                                            </td>
                                                <?php
        ...
        <?php
        if ($total_coef != 0) {
            echo $moy = sprintf("%.2f", $total_marks / $total_coef);
            /* if($query->num_rows() < 1) {
        ...
        ?>
                            <br>
                            <?php
                            $grade = $this->db->get_where('grade', array('mark_from <=' => $moy,
                                        'mark_upto >=' => $moy, 'lang' => "fr"))->row()->grade_point;
                            echo get_phrase("decision : " . $grade);
                            ?>

                            <br> <br>
                            <form  action="<?php echo site_url('admin/print_student_report_card/' . $row2['exam_id'] . '/' . $student_id . '/'); ?>" method="post">
