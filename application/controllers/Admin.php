...
class Admin extends CI_Controller {
    ...
    function marks_selector() {
        ...
        //$data['year']       = $this->db->get_where('settings' , array('type'=>'running_year'))->row()->description;
        if (
                $data['class_id'] != '' && $data['exam_id'] != '' && $data['section_id'] != ' ' && $data['subject_id'] != ''
        ) {
    ...
    function print_student_report_card($exam_id, $student_id) {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        ...
        $this->db->select('s.*, mm.moy,mm.exam_id, s.surname,  e.class_id, e.section_id');
        $this->db->join('enroll as e ', 'e.student_id = s.student_id');
        $this->db->join('mark_moy as mm ', 'mm.student_id = s.student_id');
        $this->db->where(array('e.student_id' => $student_id, 'mm.exam_id' => $exam_id, 'e.year' => $running_year, 'mm.year' => $running_year));
        $student_info = $this->db->get('student as s')->row();
        ...
        $student_note = $student_info->moy;
        //var_dump($student);die();

        $page_data['student'] = $student_info;
        $page_data['student_note'] = sprintf("%.2f", $student_note);
        ...
        $pdf_content = $this->load->view('backend/admin/report_card', $page_data, true);
        $filename = $student_info->name . '_' . $student_info->surname . '_' . $class_info->name_numeric . '_' .
                $section_name . '_' . $exam_name . '_' . $language;
        ...
        $filigrane = base_url() . "uploads/logo_filigrane.png";
        $pdf = $this->pdf->create_pdf(REPORTFOLDER, $filename, $pdf_content, '', true, false, true, '', $filigrane, $header);
        $pdf_links[] = $pdf;
        //var_dump($pdf);die();
        //$pdf_invoice = $this->pdf->create_pdf(REPORTENGLISH,$filename, $pdf_content, '', false, false,false,'',$header);
    }
