...
$edit_data		=	$this->db->get_where('enroll' , array(
	'student_id' => $param2 , 'year' => $running_year
))->result_array();
foreach ($edit_data as $row):
	$num_folder =  $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->num_dossier;
?>
<div class="row">
	<div class="col-md-12">
			...
			<div class="panel-body">
                ...
                <?php echo form_open(site_url('admin/student/do_update/'.$row['student_id'].'/'.$row['class_id'].'/'.$num_folder)  , array('class' => 'form-horizontal form-groups-bordered validate', 'enctype' => 'multipart/form-data'));?>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" name="surname" data-validate="required" data-message-required="<?php echo get_phrase('value_required');?>" 
								value="<?php echo $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->surname; ?>">
						</div>
					</div>
					...
					<div class="form-group">
						<label for="field-1" class="col-sm-3 control-label"><?php echo get_phrase('at');?></label>
                        
						<div class="col-sm-5">
							<input type="text" class="form-control" name="at" 
								value="<?php echo $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->at; ?>">
						</div>
					</div>
					...
					<div class="form-group">
						<label for="field-1" class="col-sm-3 control-label"><?php echo get_phrase('old_school');?></label>
                        
						<div class="col-sm-5">
							<input type="text" class="form-control" name="old_school"  
								value="<?php echo $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->old_school; ?>">
						</div>
					</div>
